...
class MATERIAL_PT_MSFSMaterials(Panel):
    ...
    def draw(self, context):
        ...
        if mat:
            #layout = layout.box()
            #layout.label(text="Material Mode", icon="MATERIAL")
            #layout.prop(mat, "msfs_material_mode", text="Mode")
            box=layout.box()
            ...
            if (
                mat.msfs_material_mode != "NONE"
            ):  # Just a note to anyone wondering why this isn't msfs_material_mode is not None, it's because the material type is a string enum property
                # Properties


                 # below is legacy MSFSToolkit panel
...
classes = (MATERIAL_PT_MSFSMaterials,)
